...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        # Running Header
        self.drawString(54, 750, "LearnPulse AI — Week 3 Implementation Report (Security & Access Control)")
...
def format_inline_text(text):
    # Escape standard XML/HTML entities
    text = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    parts = text.split("`")
    result = []
    for idx, part in enumerate(parts):
        if idx % 2 == 1:
            # Code block inside backticks: keep literal inside Courier font tag
            result.append(f'<font face="Courier">{part}</font>')
        else:
            # Normal text: convert **bold**
...
def build_pdf(md_path, pdf_path):
    ...
    story.append(Paragraph("WEEK 3 IMPLEMENTATION REPORT", ParagraphStyle("CovType", fontName="Helvetica-Bold", fontSize=15, textColor=colors.HexColor("#0f172a"), spaceAfter=10)))
    story.append(Paragraph("Security Architecture, User Entity Modeling, JJWT Token Engine, BCrypt Password Hashing, Spring Security Filter Chain, & Role-Based Access Control (RBAC)", ParagraphStyle("CovDesc", fontName="Helvetica", fontSize=10.5, leading=15, textColor=colors.HexColor("#475569"), spaceAfter=35)))
    ...
    meta_table_data = [
        ...
        [Paragraph("Document Title:", meta_label_style), Paragraph("Week 3 Implementation Report (Security & Access Control)", meta_val_style)],
        [Paragraph("Official Phase:", meta_label_style), Paragraph("WEEK 3 IMPLEMENTATION (Git Branch: week-3-implementation)", meta_val_style)],
        [Paragraph("Technology Stack:", meta_label_style), Paragraph("Spring Boot 3.2.5, Java 21, Spring Security 6, JJWT 0.12.5, PostgreSQL 16", meta_val_style)],
        [Paragraph("Document Version:", meta_label_style), Paragraph("1.0.0-FINAL", meta_val_style)],
        [Paragraph("Report Date:", meta_label_style), Paragraph("August 12, 2026", meta_val_style)],
        [Paragraph("Verification Status:", meta_label_style), Paragraph("COMPLETED & VERIFIED (BUILD SUCCESS — 100% Security Tests Pass)", meta_val_style)],
    ...
    while i < len(lines):
        ...
        # Ignore markdown titles that we handled in cover page
        if line.startswith("# WEEK 3 IMPLEMENTATION REPORT"):
            i += 1
            continue
...
if __name__ == "__main__":
    md_file = "/Users/krishnagarg/lms internship/WEEK_3_IMPLEMENTATION_REPORT.md"
    pdf_file = "/Users/krishnagarg/lms internship/WEEK_3_IMPLEMENTATION_REPORT.pdf"
